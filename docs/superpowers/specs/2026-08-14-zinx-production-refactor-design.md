...
10. **代码生成器 `kinzctl`（.kinz 规格文件 → 骨架代码，仿 go-zero goctl）**：作为**发布后的可选阶段 P7**，本次重构不实现，仅保留可能性（约定可枚举，见 §2.4）。
...
为给未来的代码生成工具（P7 `kinzctl`，见 §7）保留可能性，框架的约定必须保持**规则化、可枚举**：
- 路由注册只有固定形态入口（如 `AddRouterSlices(msgID, handlers...)`），消息结构定义方式统一；
- 约定（管线形状、安全默认、注册模式）以文档形式沉淀为单一事实来源（P5）；
- `kinzctl` 只依赖 `kiface` 约定 + 文档，不依赖框架内部实现，作为发布后的独立增量。
...
> **本次重构不实现**，v1.0.0 发布后按需启动；写进计划是为保住可能性（约定可枚举，§2.4）。

- 内容：仿 go-zero goctl——`.kinz` 规格文件（服务配置 + 消息定义 + 路由声明）→ `kinzctl` 一键生成骨架代码（消息结构体、路由注册、handler 桩、客户端桩），用户只填业务逻辑；独立模块（不依赖框架内部实现，只依赖 `kiface` 约定与文档）。
- 前置条件：P5 文档已固化约定为单一事实来源；框架 API 已稳定（P6 之后）。
- 新增测试：生成代码可编译；生成代码与手写代码行为一致（golden 文件对比）。
- 退出标准：`kinzctl gen` 从 `.kinz` 文件生成可编译骨架；生成的 ping/chatroom 示例可运行。
